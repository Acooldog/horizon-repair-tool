            ...
            foreach (var domain in xboxDomains)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        combinedResult.AddIssue($"DNS解析失败: {domain}", "无法解析域名", async (p) => await RepairNetworkConfig(p));
                    }
                }
                catch (Exception ex)
                {
                    Logs.LogInfo($"域名 {domain} 解析异常: {ex.Message}");
                    combinedResult.AddIssue($"DNS解析异常: {domain}", ex.Message, async (p) => await RepairNetworkConfig(p));
                }
            }
