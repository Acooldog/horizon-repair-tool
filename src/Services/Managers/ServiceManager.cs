...
{   
    public class ServiceManager
    {
            ...
            Action<int, string>? ProgressCount = null)
        {
            ...
            UnEnableProgress.fullCount = serviceNames.Length;
            ...
            foreach (string name in serviceNames)
            {
                ...
                finally
                {
                    ...
                    ProgressCount?.Invoke(
                            ( (successCount + failedCount + notExistCount + nameResolveFailedCount) / UnEnableProgress.fullCount ) * 100,
                            $"{((successCount + failedCount + notExistCount + nameResolveFailedCount) / UnEnableProgress.fullCount) * 100}%"
                        );
                }
            }
